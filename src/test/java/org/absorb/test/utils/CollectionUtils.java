...
public final class CollectionUtils {
    ...
    public static <T> void findMatch(Collection<T> expected, Collection<?> incoming) {
        if (expected.stream().allMatch(value -> value instanceof Comparable)) {
            Collection<Comparable<?>> expectedString = expected
                    .stream()
                    .map(value -> (Comparable<T>) value)
                    .collect(Collectors.toSet());
            Collection<Comparable<?>> incomingString = incoming
                    .stream()
                    .map(value -> (Comparable<T>) value)
                    .collect(Collectors.toSet());
        ...
        if (expected.stream().allMatch(value -> value instanceof Map<?, ?>) && expected
                .stream()
                .map(value -> (Map<?, ?>) value)
                .allMatch(map -> map.containsKey("id"))) {
            Collection<Map<?, ?>> expectedMap = expected
                    .stream()
                    .map(value -> (Map<?, ?>) value)
                    .collect(Collectors.toSet());
            Collection<Map<?, ?>> incomingMap = incoming
                    .stream()
                    .map(value -> (Map<?, ?>) value)
                    .collect(Collectors.toSet());
    ...
    public static <T> void match(Collection<T> expected, Collection<T> incoming, Comparator<T> comparator)
            throws IllegalArgumentException {
        if (expected.size() > incoming.size()) {
            throw new IllegalArgumentException(
                    "Expected was larger then incoming: Expected(" + expected.size() + "):" + " Incoming("
                            + incoming.size() + ")");
        }
        ...
        for (int a = 0; a < expected.size(); a++) {
            ...
            if (expectedValue instanceof Collection<?> expectedCollection) {
                ...
                throw new IllegalArgumentException(
                        "Expected a Collection however got " + incomingValue.getClass().getName());
            }
    ...
    public static void match(Map<?, ?> expected, Map<?, ?> incoming) throws IllegalArgumentException {
        for (Map.Entry<?, ?> entry : expected.entrySet()) {
            if (!incoming.containsKey(entry.getKey())) {
                throw new IllegalArgumentException(
                        "Could not find key of '" + entry.getKey() + "' in " + incoming.entrySet());
            }
            ...
            if (entry.getValue() instanceof Collection<?> collection) {
                if (!(value instanceof Collection<?> result)) {
                    throw new IllegalArgumentException(
                            "The value of '" + entry.getKey() + "' was excepted to be a " + "Collection, however was a "
            ...
            if (entry.getValue() instanceof Map<?, ?> map) {
                if (!(value instanceof Map<?, ?> result)) {
                    throw new IllegalArgumentException(
                            "The value of '" + entry.getKey() + "' was excepted to be a " + "Map, however was a "
                                    + value.getClass().getName());
                }
                try {
                    match(map, result);
                    ...
                    throw new IllegalArgumentException(
                            "Exception on key " + entry.getKey() + " -> map matching\nMap " + "found in list with "
                                    + expected.entrySet(), e);
                }
                continue;
            }
            if (!entry.getValue().equals(value)) {
                throw new IllegalArgumentException(
                        "The value of '" + entry.getKey() + "' does not match \n'" + entry.getValue() + "' as '" + entry
                                .getValue()
                                .getClass()
