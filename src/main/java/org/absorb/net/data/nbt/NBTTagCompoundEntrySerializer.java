...
public class NBTTagCompoundEntrySerializer implements Serializer<NBTCompoundEntry<?, ?>> {

    @Override
    public NetEntryData<NBTCompoundEntry<?, ?>> read(int position, ByteBuffer bytes) {
        NetEntryData<Byte> id = Serializers.BYTE.read(position, bytes);
        NetEntryData<String> key = Serializers.STRING.read(id.endingPosition(), bytes);
        try {
    ...
    public ByteBuffer write(NBTCompoundEntry<?, ?> value) {
        ByteBuffer idBuffer = Serializers.BYTE.write((byte) value.getTag().getId());
        ByteBuffer keyBuffer = Serializers.SHORT_STRING.write(value.getKey().getKey());
        ...
        System.out.println("\t\tKey: " + Serializers.SHORT_STRING.read(0, ByteBuffer.wrap(keyBuffer.array())).value());
