...
public class ShortDefinedStringSerializer implements Serializer<String> {

    @Override
    public NetEntryData<String> read(int position, ByteBuffer buffer) {
        NetEntryData<Short> length = Serializers.UNSIGNED_SHORT.read(position, buffer);
    ...
    public ByteBuffer write(String value) {
        try {
            ...
            ByteBuffer lengthBuffer = Serializers.UNSIGNED_SHORT.write((short) bytes.length);
