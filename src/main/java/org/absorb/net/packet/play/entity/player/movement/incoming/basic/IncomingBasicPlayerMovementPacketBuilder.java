package org.absorb.net.packet.play.entity.player.movement.incoming.basic;
...
public class IncomingBasicPlayerMovementPacketBuilder implements IncomingPacketBuilder<IncomingBasicPlayerMovementPacket> {
    ...
    public PacketBuilder<IncomingBasicPlayerMovementPacket> from(ByteBuffer packetBytes) {
        NetEntryData<Double> x = Serializers.DOUBLE.read(0, packetBytes);
        NetEntryData<Double> y = Serializers.DOUBLE.read(x.endingPosition(), packetBytes);
        NetEntryData<Double> z = Serializers.DOUBLE.read(y.endingPosition(), packetBytes);
        NetEntryData<Boolean> onGround = Serializers.BOOLEAN.read(z.endingPosition(), packetBytes);
