package org.absorb.net.packet.play.settings.client;
...
public class IncomingClientSettingsPacketBuilder implements IncomingPacketBuilder<IncomingClientSettingsPacket> {
    ...
    public PacketBuilder<IncomingClientSettingsPacket> from(ByteBuffer packetBytes) {
        NetEntryData<String> localeString = Serializers.STRING.read(0, packetBytes);
        NetEntryData<Byte> viewDistance = Serializers.BYTE.read(localeString.endingPosition(), packetBytes);
        NetEntryData<Integer> chatModeId = Serializers.VAR_INTEGER.read(viewDistance.endingPosition(), packetBytes);
        NetEntryData<Boolean> chatColoursEnabled = Serializers.BOOLEAN.read(chatModeId.endingPosition(), packetBytes);
        NetEntryData<Byte> skinPartsId = Serializers.BYTE.read(chatColoursEnabled.endingPosition(), packetBytes);
        NetEntryData<Integer> mainHandId = Serializers.VAR_INTEGER.read(skinPartsId.endingPosition(), packetBytes);
        NetEntryData<Boolean> textFiltering = Serializers.BOOLEAN.read(mainHandId.endingPosition(), packetBytes);
        NetEntryData<Boolean> showOnList = Serializers.BOOLEAN.read(textFiltering.endingPosition(), packetBytes);
